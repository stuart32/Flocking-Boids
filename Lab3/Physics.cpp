...
void Physics::addNeighbour(int n) {
	
	if (neighNo < 3) {
	neighbours[neighNo] = n;
	neighNo++;
	}
...
void Physics::calcPosition(float time) {
	if (position.x < -25.0f || position.x > 25.0f
		|| position.y < -25.0f || position.y > 25.0f
		|| position.z > 0.0f || position.z < -50.0f) {
		position = vec3(0.0f, 0.0f, -20.0f);
		netForce = vec3((rand() % 1500 - 750) / 10000.0f, (rand() % 1500 - 750) / 10000.0f, (rand() % 1500 - 750) / 10000.0f);
	}
